...
function normalizeRowId<T>(data: T): T {
  if (Array.isArray(data)) {
    return data.map((item) => normalizeRowId(item)) as T
  }

  if (data && typeof data === 'object') {
    const record = data as Record<string, unknown>

    if ('_row_id' in record && !('row_id' in record)) {
      return { ...record, row_id: record._row_id } as T
    }
  }
...
export async function callFunction(functionId: string, input: Record<string, unknown> = {}): Promise<ApiResult> {
  try {
    ...
    return {
      ...result,
      data: normalizeRowId(result.data),
    }
  } catch (error) {
